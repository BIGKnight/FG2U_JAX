...
from metaoptim import inner_step, loss_func, acc, grad_func
...
def construct_random_data(
    ...
    if not load_checkpoint:
        random_images = jnp.ones([num_classes * ipc, img_size, img_size, num_channels]) * 0.5
...
parser.add_argument('--truncated_steps', type=int, default=0)
...
parser.add_argument('--wandb_expname', type=str, default='reverse_ipc1_cifar_inner100')
...
if __name__ == "__main__":
    ...
    print("Inner batch size: ", inner_batch_size)
    ...
    while total_steps < args.num_steps:
        ...
        if total_steps % args.eval_step == 0:
            ...
            for i in range(args.eval_num_inner_steps):
                ...
                eval_params, eval_inner_opt_state, _, _ = inner_step(
                    eval_params,
            ...
            meta_losses, meta_acces, test_acces = [], [], []
        ...
        with logger.measure_time('step'):
            batch = next(batches)
            rng, key = random.split(rng)
            init_inner_params = init(key, model, img_size, num_channels)
            init_inner_opt_state = inner_optimizer.init(init_inner_params)
            inner_params = init_inner_params
            inner_opt_state = init_inner_opt_state
            for i in range(args.truncated_steps):
                rng, key = random.split(rng)
                random_indexes = jax.random.permutation(rng, len(condensated_images))[:inner_batch_size]
                mini_params = condensated_images[random_indexes]
                mini_labels = condensated_labels[random_indexes]
            ...
            g_approx = grad_func(
                ...
                inner_params,
                inner_opt_state,
                args.num_inner_steps - args.truncated_steps, 
                condensated_images, 
                condensated_labels,
                batch,
                Partial(model.apply),
