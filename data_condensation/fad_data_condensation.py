...
from metaoptim import hypergrad_approx, inner_step, loss_func, acc
...
def init(key, model, img_size=32, num_channels=3): 
...
parser.add_argument('--num_truncated_steps', type=int, default=0)
parser.add_argument('--eval_num_inner_steps', type=int, default=100)
parser.add_argument('--num_steps_per_savepoint_update', type=int, default=1)
parser.add_argument('--num_inner_steps', type=int, default=100)
parser.add_argument('--meta_optimizer', type=str, default='adam')
parser.add_argument('--meta_decay_scheduler', type=str, default='linear')
parser.add_argument('--inner_decay_scheduler', type=str, default='constant')
...
parser.add_argument('--num_vs', type=int, default=10)
parser.add_argument('--num_vs_vmap', type=int, default=5)
parser.add_argument('--v_dist', type=str, default='rademacher')  # 'normal', 'rademacher'
parser.add_argument('--wandb', action='store_true')
parser.add_argument('--wandb_expname', type=str, default='cifar_data_condensation')
...
logger = Logger(
    'nvs={}_trunc={}_update={}_seed={}'.format(
        args.num_vs, args.num_truncated_steps, 
        args.num_steps_per_savepoint_update, 
        args.seed
    ),
    ...
    while total_steps < args.num_steps:
        ...
        if total_steps % args.eval_step == 0:
            ...
            for i in range(args.eval_num_inner_steps):
                rng, key = random.split(rng)
                random_indexes = jax.random.permutation(key, len(condensated_images))[:inner_batch_size]
        ...
        with logger.measure_time('step'):
            num_inner_steps = args.num_inner_steps - args.num_truncated_steps
            batch = next(batches)
            rng, key = random.split(rng)
            inner_params = init(key, model, img_size, num_channels)
            inner_opt_state = inner_optimizer.init(inner_params)    

            g_approx = hypergrad_approx(
                rng,
                args.num_vs, args.num_vs_vmap, 
                inner_params,
                inner_opt_state,
                num_inner_steps, 
                condensated_images, 
            ...
            if not args.no_clip:
                condensated_images = condensated_images.clip(0., 1.)
        total_steps += 1
        pbar.update(1)
